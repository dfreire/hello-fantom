...
fan.web.WebUtil.fromQuotedStr = function(s)
{
  ...
  ;
  return fan.sys.Str.replace(fan.sys.Str.slice(s,fan.sys.Range.make(1,-2)),"\\\"","\"");
}
...
fan.web.WebUtil.parseHeaders = function($in)
{
  ...
  while (true)
  {
    ...
    {
      headers = fan.sys.Str.plus(headers.get(fan.sys.ObjUtil.coerce(last,fan.sys.Str.$type)),fan.sys.Str.plus(" ",fan.sys.Str.trim($in.readLine())));
      continue;
    }
    ;
...
fan.web.WebUtil.jsMain = function(out,main,env)
{
  ...
  if (fan.sys.ObjUtil.compareGT((function($this) { var $_u0 = env; if ($_u0 == null) return null; return $_u0.size(); })(this),0))
  {
...
fan.web.WebUtil.static$init = function()
{
  if (true)
  {
    ...
    fan.web.WebUtil.m_tokenChars = fan.sys.ObjUtil.coerce((function($this) { var $_u1 = m; if ($_u1 == null) return null; return fan.sys.ObjUtil.toImmutable($_u1); })(this),fan.sys.Type.find("sys::Bool[]"));
  }
  ;
...
fan.web.Cookie.fromStr = function(s)
{
  ...
  ;
  var name = fan.sys.Str.trim(fan.sys.Str.slice(s,fan.sys.Range.make(0,fan.sys.ObjUtil.coerce(eq,fan.sys.Int.$type),true)));
  var val = fan.sys.Str.trim(fan.sys.Str.slice(s,fan.sys.Range.make(fan.sys.Int.plus(fan.sys.ObjUtil.coerce(eq,fan.sys.Int.$type),1),-1)));
